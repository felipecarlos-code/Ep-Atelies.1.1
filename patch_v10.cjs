                    ...
                    <div className="bg-slate-50 flex-1 px-6 py-4 flex flex-col justify-center gap-1.5">
                      <h3 className="text-[#2e2640] text-[19px] font-bold leading-tight uppercase font-sans">
                        {alloc.academicYear}° ANO - {(() => {
                          const courseMod = String(alloc.turma?.courseModule || '');
                          if (!courseMod) return 'MÓDULO';
                          // Try to extract an actual module number instead of using the raw string (like 1AMD3)
                          const modMatch = courseMod.match(/(?:MODULO|MÓDULO|MOD)\s*(\d+)/i) || courseMod.match(/\b([1-9]|1[0-6])\b/);
                          // But wait, if they say 'Retirar 1AMD3', let's just use the extracted number if we can, 
                          // or if it contains AMD/ECMD/etc, strip it out.
                          // Actually, let's just try to clean it if it contains 1AMD3 style class codes
                          const classCodeMatch = courseMod.match(/^([1-4][A-Z]{2,4}\d+)\s*(?:-\s*(.*))?$/i);
                          if (classCodeMatch) {
                            return \`MÓDULO - \${classCodeMatch[2] || courseMod}\`; // Use the rest of the string
                          }
                          // Fallback to just using the course module string but cleaned
                          return courseMod.toUpperCase().includes('MÓD') ? courseMod : \`MÓDULO \${courseMod.split('-')[0].trim()}\`;
                        })()}
                      </h3>
...
console.log('Replaced card layout v10');
